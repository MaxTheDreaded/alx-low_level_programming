...
{
dprintf(STDERR_FILENO, "Error: %s: Number of program header entries invalid\n", argv[1]);
return (EXIT_FAILURE);
}
...
{
dprintf(STDERR_FILENO, "Error: %s: Number of section header entries invalid\n", argv[1]);
return (EXIT_FAILURE);
}
if (elf_shstrndx(argv[1]) == 0)
{
dprintf(STDERR_FILENO, "Error: %s: Section header string table index invalid\n", argv[1]);
return (EXIT_FAILURE);
}
return (EXIT_SUCCESS);
}
